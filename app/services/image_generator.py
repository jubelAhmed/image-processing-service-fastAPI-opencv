...
class ImageGenerator:
    ...
    def create(self, image_shape: Tuple[int, int], regions: List[List[List[int]]], processed_image: Optional[np.ndarray] = None) -> str:
        if not self._validate_inputs(image_shape, regions):
            raise ValueError("Invalid input parameters")
        return self.generator.generate(image_shape, regions, processed_image)

    def set_generator(self, generator: OutputGenerator) -> None:
        self.generator = generator

    def _validate_inputs(self, image_shape: Tuple[int, int], regions: List[List[List[int]]]) -> bool:
